  ...
  <div class="admin-students">
    <div class="header">
      ...
      <button class="btn btn-primary" @click="openAddModal">
        <i class="fas fa-plus"></i> Dodaj Ucznia
      </button>
    </div>

    <div v-if="students.length === 0" class="no-data">
      Brak uczniów w bazie danych
    </div>
    ...
    <table v-else class="students-table">
      ...
      <tbody>
        <tr v-for="student in students" :key="student.id">
          ...
          <td class="actions">
            <button class="btn btn-edit" @click="openEditModal(student)">
...
export default {
  ...
  setup() {
    ...
    const modalTitle = computed(() =>
      isEditing.value ? 'Edytuj Ucznia' : 'Dodaj Ucznia'
    );
    ...
    const fetchStudents = async () => {
      try {
        ...
        alert('Nie udało się pobrać listy uczniów');
      }
    };
    ...
    const submitForm = async () => {
      ...
      const url = isEditing.value
        ? `http://localhost:8000/api/students/${selectedStudent.value.id}/update/`
      ...
      try {
        ...
        alert('Nie udało się zapisać ucznia');
      } finally {
    ...
    const deleteStudent = async () => {
      ...
      try {
        await axios.delete(
          `http://localhost:8000/api/students/${selectedStudent.value.id}/delete/`,
          {
            headers: { Authorization: `Bearer ${localStorage.getItem('access')}` }
          }
        );
...
/* Reszta stylów jest taka sama jak w AdminInstructors */
</style>